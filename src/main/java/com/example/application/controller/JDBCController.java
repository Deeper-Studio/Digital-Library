...
//        objects[7] = "staff";
    ...
    //选定3个评审
    @GetMapping("/judge")
    public Object[] judgeList(){
        String sql = "select  uuid,name from users.user order by rand() limit 3";
        Object[] list = jdbcTemplate.queryForList(sql).toArray();
        updateLevel(list);
        return list;
    }

    //Recycle the permission, setting the judgement column to null
    public String complete(String uuid){

        String sql = "update users.user set judgement = null where user.uuid = ?";;
        jdbcTemplate.update(sql,uuid);
        return "complete";
    }

    //添加权限，根据uuid
    public String[] updateLevel(Object[] list){
        String[] origin = new String[3];
        for(int i = 0; i < 3; i++) {
            String sql = "update users.user set judgement = 'judge' where user.uuid = ?";;
            jdbcTemplate.update(sql,list[i].toString().substring(6,38));
            origin[i] = list[i].toString().substring(6,38);
        }
        return origin;
    }
}
